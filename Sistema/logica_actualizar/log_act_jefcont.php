<?php
    if (isset($_GET['num_oc'])) {
        ...
        $sql1 = "INSERT INTO    bd_contadores (num_oc, fecha_emision, empresa, unidad_de_negocio, 
                                ...
                                referencia FROM bd_jef_cont WHERE num_oc = '$num_oc'";

        $sql2 = "INSERT INTO    bd_contadores (rec_cont) VALUE ('$rec_cont')";
...
//---------------------------------RECIBE CONT = NO---------------------------------------------------------------------        
        ...
        $sql2 = "UPDATE bd_contadores SET rec_cont = ('$rec_cont') WHERE num_oc = '$num_oc'";
...
//---------------------------------RECIBE JEF CONT = SI---------------------------------------------------------------------
        ...
        $sql3 = "DELETE FROM bd_jef_cont WHERE num_oc = '$num_oc'";
...
//----------------------------HORA DE ACTUALIZACION Y CONFIRMACION BASE GENERAL--------------------------------------------------------------------------
        ...
        $fecha_rec_jefcont = date("Y-m-d");

        $sql4 = "UPDATE datos_oc SET rec_jefcont = ('$rec_jefcont'), fecha_rec_jefcont = ('$fecha_rec_jefcont') WHERE num_oc = '$num_oc'";
  
        mysqli_query($conexion_adq4,$sql4);
        $conexion_adq4->close();
//------------------------------------------------------------------------------------------------------
        header("location: ../acciones_admin/actualizar_jefcont.php?rol=Administrador&area=Jefatura Contabilidad");
    } 
?>