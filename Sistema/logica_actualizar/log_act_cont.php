<?php
    if (isset($_GET['num_oc'])) {
        ...
        $sql1 = "INSERT INTO    bd_tesoreria (num_oc, fecha_emision, empresa, unidad_de_negocio, 
                                ...
                                referencia FROM bd_contadores WHERE num_oc = '$num_oc'";
...
//---------------------------------RECIBE TESORERIA = NO---------------------------------------------------------------------        
        ...
        $sql2 = "UPDATE bd_tesoreria SET rec_tes = ('$rec_tes'), pagado = ('$pagado') WHERE num_oc = '$num_oc'";
...
//---------------------------------RECIBE CONT = SI---------------------------------------------------------------------
        ...
        $sql3 = "DELETE FROM bd_contadores WHERE num_oc = '$num_oc'";
...
//----------------------------HORA DE ACTUALIZACION Y CONFIRMACION BASE GENERAL--------------------------------------------------------------------------
        ...
        $sql4 = "UPDATE datos_oc SET rec_cont = ('$rec_cont'), fecha_rec_cont = ('$fecha_rec_adq') WHERE num_oc = '$num_oc'";

        mysqli_query($conexion_adq4,$sql4);
        $conexion_adq4->close();        
//------------------------------------------------------------------------------------------------------------------------------
        header("location: ../acciones_admin/actualizar_cont.php?rol=Administrador&area=Contabilidad");
    } 
?>